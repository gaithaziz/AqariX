...
LANDMARKS = (
    ...
    LandmarkSignal(
        ...
        latitude=32.495,
        longitude=35.9912,
        aliases=("جامعة العلوم والتكنولوجيا", "التكنو", "just", "جست"),
    ),
    LandmarkSignal(
        key="kauh",
...
def _extract_intent(text: str) -> ListingIntent:
    if _contains_any(text, ("للايجار", "ايجار", "اجار", "للإيجار")):
...
def _extract_furnished(text: str) -> bool | None:
    if _contains_any(text, ("غير مفروش", "بدون فرش")):
        return False
    if _contains_any(text, ("مفروش", "فرش كامل", "مع فرش")):
        return True
    return None
